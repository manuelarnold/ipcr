...
fit <- lavaan::lavaan(model = m, data = data_model)

fit@Fit@Sigma.hat
cov(data_model)


fit_mx <- OpenMx::mxTryHard(x_mx)

res_mx <- ipcr(fit = fit_mx, covariates = covariates, iterate = TRUE, regularization = TRUE)
res <- ipcr(fit = fit, covariates = covariates, iterate = TRUE)

coeftest(res)

sapply(res$regression_list, coeftest)

a <- coeftest(res$regression_list$`y~x`, )
a


# S4 methods
# AIC coef effects fitted logLik nobs predict residuals sigma vcov
AIC(res)
AIC(res, parameter = "y~x")
BIC(res)
BIC(res, parameter = "y~x")
coef(res)
coef(res, parameter = "y~x")
effects(res)
effects(res, parameter = "y~x")
fitted(res)
fitted(res, parameter = "y~x")
logLik(res)
logLik(res, parameter = "y~x")
nobs(res)
nobs(res, parameter = "y~x")
predict(res)
predict(res, parameter = "y~x")
residuals(res)
residuals(res, parameter = "y~x")
sigma(res)
sigma(res, parameter = "y~x")
vcov(res)
vcov(res, parameter = "y~x")

a <- sapply(res$regression_list, FUN = effects)

help("effects")
res$info
res_mx$info

res_mx$output$regularized_coefficients_matrix
res_mx$output$regularized_coefficients_matrix


sapply(res_mx$regression, coef)
sapply(res$regression, coef)


res_mx$iteration_matrix
res$iteration_matrix
...
res1 <- ipcr(fit = x_mx, covariates = covariates, iterate = FALSE, linear = TRUE)
res2 <- ipcr(fit = x_mx, covariates = covariates, iterate = TRUE, linear = FALSE)

predict(res1)

setOldClass("ipcr")

setGeneric("predict_manu", function(object, ...) standardGeneric("predict_manu"))

setMethod("predict_manu", signature(object = "ipcr"),
          function(object, ...){
            print(object$info)
          })
...
res1 <- ipcr(fit = x, covariates = covariates, iterated = TRUE, linear = TRUE)
res2 <- ipcr(fit = x, covariates = covariates, iterated = TRUE, linear = FALSE)
...
rm(ipcr:::summary.ipcr)
ipcr:::coef.ipcr(res1)
ipcr:::summary.ipcr(res1)
