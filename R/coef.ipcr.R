#' @title Extract Individual Parameter Regression Estimates
#' @description This function extracts the individual parameter regression estimates from an ipcr object.
...
coef.ipcr <- function(object, parameter = NULL, method = "iterated", ...) {
  ...
  if (method == "standard") {
    res <- sapply(X = object$regression, FUN = coef)
  } else {
    res <- sapply(X = object$iterated_regression, FUN = coef)
  }
