...
#'
#' # Run the model
#' IPC_reg <- ipcr(fit = Fit, formula = ~ z1 + z2, covariates = ipc_data, iterate = T)
#'
#' # Display the iterated individual parameter contribution regression estimates of beta
#' summary(object = IPC_reg, parameter = "beta", method = "iterate")
#' @export ipcr

ipcr <- function(fit, formula = NULL, covariates = NULL, iterate = F,
  ...
  centered_data <- scale(x = data_obs, center = T, scale = F)
  ...
  jacobian <- suppressMessages(expr = mxCheckIdentification(model = fit, details = F)$jacobian)
  if (mean_structure == F) {
  ...
  initial_IPC <- matrix(data = rep(x = param_estimates, times = N),
                        byrow = T, nrow = N, ncol = q) + moment_deviations %*% t(W)
  initial_IPC <- as.data.frame(x = initial_IPC)

  # Store the results
  IPC <- list("StandardIPCs" = initial_IPC)

  # Status report
  cat("Standard IPCs computed\n")
  ...
  # Store the results
  IPC[[2]] <- IPC_reg_list
  names(IPC)[2] <- "StandardIPCRegression"

  # Status report
  cat("Standard IPC regression parameters estimated\n")
  ...
  if(iterate) {
    ...
    for (i in 1:N) {
      if (is.na(group[i, 2])) {
        for (j in i:N) {
          if (all(covariates[j, , drop = F] == covariates[i, , drop = F])) {
    ...
    while(nr_iterations < max_it & isFALSE(all(abs(difference) < convergence_criterion))){
      ...
      for (i in unique(group[, 2])) {
        ...
        fit_up[[i]] <- suppressMessages(mxRun(model = fit_up[[i]], useOptimizer = F))
        suppressMessages(jacobian_up <- mxCheckIdentification(model = fit_up[[i]], details = F)$jacobian)
        if (mean_structure == F) {
        ...
        moment_deviations_up[ID_group, 1:p_star] <- mom_cont_igsmc[ID_group, 1:p_star] -
          matrix(data = rep(x = vech(expected_cov_up), times = n_group), byrow = T,
        ...
        updated_IPC[ID_group, ] <- moment_deviations_up[ID_group, ] %*% t(W_up) +
          matrix(data = rep(x = param_up, times = n_group), byrow = T, nrow = n_group, ncol = q)
      }
