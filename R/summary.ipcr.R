...
summary.ipcr <- function(object, what = "everything", digits = 3, ...) {
  ...
  cat("  MANCOVA:\n")
  mancova <- as.data.frame(round(object$output$mancova, digits = digits))
  mancova["Residuals", 2:6] <- ""
  mancova <- cbind(Predictor = rownames(mancova), mancova)
  mancova <- rbind(colnames(mancova), mancova)
  ...
  F_tests_output <- rbind(colnames(F_tests_output), F_tests_output)
  ...
  ipcr_output <- rbind(colnames(ipcr_output), ipcr_output)
