...
summary.ipcr <- function(object, parameter = NULL, method = "both") {
  ...
  if (!any(c("StaticIPCRegression", "IteratedIPCRegression") %in% names(object))) {
    cat("The ipcr object does not contain any estimated IPC regression parameters")
  } else {
    if (method == "iterated" & !any("IteratedIPCRegression" %in% names(object))) {
      cat("Iterated IPC regression parameters have not been estimated")
    } else {
      ...
      if (method %in% c("iterated", "both") & any("IteratedIPCRegression" %in% names(object))) {
